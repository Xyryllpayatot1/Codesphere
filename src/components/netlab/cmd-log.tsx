...
/** Scrollable command history — shared by the desktop dock and the mobile sheet. */
export function CmdLogList() {
  const cmdLog = useNetlab((s) => s.cmdLog);

  if (cmdLog.length === 0) {
    return (
      <p className="pt-2 text-xs text-muted-foreground">
        No commands yet — open a device&apos;s Command Prompt (double-click it, then the Command Prompt tab) and run ping,
        ipconfig, arp&hellip;
      </p>
    );
  }
    ...
    <div className={cn("shrink-0 border-t border-border bg-card/70 backdrop-blur", open ? "h-44" : "h-9")}>
      <div className="flex h-9 items-center gap-2 px-3">
        <button
          ...
          aria-expanded={open}
          className="flex items-center gap-1.5 rounded-md px-2 py-1 text-xs font-medium text-muted-foreground hover:bg-secondary"
        >
