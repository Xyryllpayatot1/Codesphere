...
type Example = Extract<ContentBlock, { type: "example" }>;

const PANELS = [
  { key: "html", label: "HTML", monaco: MONACO_LANGUAGE.html, get: (e: Example) => e.html ?? "" },
  { key: "css", label: "CSS", monaco: MONACO_LANGUAGE.css, get: (e: Example) => e.css ?? "" },
  { key: "js", label: "JavaScript", monaco: MONACO_LANGUAGE.javascript, get: (e: Example) => e.js ?? "" },
] as const;

export function ExampleBlock({ block }: { block: Example }) {
    ...
    <div className="my-6 overflow-hidden rounded-xl border border-border">
      <div className="flex items-center justify-between gap-2 border-b border-border bg-card px-4 py-2.5">
        <div className="min-w-0">
          <p className="truncate text-sm font-semibold">{block.title || "Try it yourself"}</p>
      ...
      <div className="grid gap-0 lg:grid-cols-2">
          ...
          {active.map((p) =>
            p.key === activeTab ? (
              <CodeEditor
                ...
                height={Math.max(240, block.height ?? 280)}
        ...
        <div className="relative min-h-[320px] bg-white">
          ...
          <iframe key={previewKey} title={block.title ?? "Example preview"} srcDoc={srcDoc} sandbox="allow-scripts allow-modals" className="h-full min-h-[320px] w-full" />
        </div>
      </div>
    </div>
  );
