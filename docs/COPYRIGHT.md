# CodeSphere Copyright & Ownership
...
This document establishes the ownership and attribution record for CodeSphere. It describes which materials are project-owned, which are third-party, and how the project has been developed. It is intended as supporting documentation should the creator choose to pursue formal copyright registration or other intellectual-property protection in the future.
...
The following categories contain materials created for CodeSphere and are owned by the project:

- **Project concept and product identity** — the CodeSphere name, mission, vision, and platform definition
- **Branding and original graphics** — the CodeSphere visual identity, logo assets, and PWA iconography created for the project (see `public/` and `scripts/generate-pwa-assets.mjs`)
...
- **Original source code** — application code written for CodeSphere under `src/` (see Section 2)
- **Original UI/UX design** — layout, component design, and interaction patterns created for CodeSphere (see Section 3)
...
Original materials are created specifically for CodeSphere. Not every file in the repository is original to CodeSphere; third-party and open-source components are identified separately in Section 10 and in `THIRD_PARTY.md`.
...
- `src/components/` — React components built for CodeSphere
- `src/lib/` — shared libraries, business logic, services, and utilities

The following are **not** original CodeSphere materials and remain the property of their respective owners:
...
The user interface and user experience design — including the component design system, layout structure, navigation patterns, visual styling, dark/light theming, and the overall look and feel of the platform — are original project-owned material created for CodeSphere.
...
CodeSphere's learning content is original to the project, including:
...
The virtual networking laboratory is a core original component of CodeSphere. Its architecture — including device models, link/cable models, packet flow, network simulation state management, and mission/troubleshooting design — is original project-owned material (see `src/lib/net/`).
...
Interactive games and educational exercises built for CodeSphere — their game design, level definitions, grading rules, and content — are original project-owned material (see the games and learn modules under `src/`).
...
Original graphics created for CodeSphere — including the CodeSphere logo, PWA icons, and any in-app artwork — are project-owned. Branding assets are generated from project scripts in `scripts/` and stored under `public/`.
...
CodeSphere is built on widely used open-source technologies and libraries. These remain the property of their respective authors and are not claimed as CodeSphere material. Major third-party components include (not exhaustive):
...
CodeSphere and its original materials are held under an all-rights-reserved copyright notice (see the notice at the top of this document). The project currently does **not** publish its own open-source license.
...
CodeSphere was developed using AI-assisted software development tools alongside human-led product design, architecture, testing, and development direction. This means parts of the implementation were produced with AI assistance under the guidance of the project creator.
...
The development history of CodeSphere is tracked through the project's Git repository and summarized in `DEVELOPMENT_HISTORY.md`. A versioned record of milestones, features, and commits is maintained in `DEVELOPMENT_RECORD.md`.
